...
db norm(db x, db l = 0, db r = 2 * M_PI) {  // change x into [l,r)
...
VVP ConvexHull3D(VP _p) {
  ...
  for (int i = 2; i < n; i++)
    if ((q[i].x - q[0].x) * (q[1].y - q[0].y) > (q[i].y - q[0].y) * (q[1].x - q[0].x)) swap(q[1], q[i]), swap(p[1], p[i]);
  wrap(0, 1);
  return ret;
}
}  // namespace CH3
...
void solve() {
  ...
  for (int i = 0; i < A.size(); ++i) {
    ...
    for (int j = i + 1; j < A.size(); ++j)
      if (cmp(A[i].o.dis(A[j].o), A[j].r - A[i].r) <= 0) {
        flag = 1;
        break;
      }
    if (!flag) {
  ...
  A.resize(n);
  if (A.size() == 1) {
    double ans = 2 * M_PI * A[0].r;
    printf("%.11lf\n", ans);
    return;
  }
  for (int i = 0; i < A.size(); ++i)
    for (int j = 0; j < i; ++j) {
      vector<line> x = TangentoutCC(A[i], A[j]);
      for (int k = 0; k < x.size(); ++k) {
        for (int a = 0; a < 2; a++) {
          point k1 = x[k][a];
          bool flag = 0;
          for (int b = 0; b < A.size(); b++)
            if (A[b].inside(k1) == 1) flag = 1;
          if (flag == 0) B.push_back(k1);
        }
      }
    }
  // cout<<B.size()<<endl;
  B = ConvexHull(B);
  B.push_back(B[0]);
  //bool bo=(B.size()==35672);
  /*if (bo){
        for (int i=0;i<B.size();i++) cout<<(double)B[i].x<<" "<<(double)B[i].y<<" "<<getpos(B[i])<<endl;
    }*/
  double ans = 0;
  for (int i = 1; i < B.size(); ++i) {
    int k1 = getpos(B[i - 1]), k2 = getpos(B[i]);
    if (k1 != k2)
      ans += B[i - 1].dis(B[i]);
    else {
      double l = (B[i - 1] - A[k1].o).getw();
      double r = (B[i] - A[k1].o).getw();
      if (cmp(l, r) == 1) r += 2 * M_PI;
      ans += (r - l) * A[k1].r;
    }
  }
  printf("%.11lf\n", ans);
